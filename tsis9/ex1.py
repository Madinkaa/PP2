...
last_figure = (400, 300)  # Initial position at the center of the screen
move_speed = 5
...
while True:
    ...
    for event in pygame.event.get():
        print(event)  # Debugging: Print events to console
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            elif event.key == pygame.K_UP and last_figure:  # Move up
                last_figure = (last_figure[0], last_figure[1] - move_speed)
            elif event.key == pygame.K_DOWN and last_figure:  # Move down
                last_figure = (last_figure[0], last_figure[1] + move_speed)
            elif event.key == pygame.K_LEFT and last_figure:  # Move left
                last_figure = (last_figure[0] - move_speed, last_figure[1])
            elif event.key == pygame.K_RIGHT and last_figure:  # Move right
                last_figure = (last_figure[0] + move_speed, last_figure[1])
    ...
    # Draw the last drawn figure if available
    if last_figure:
        pygame.draw.circle(screen, RED, last_figure, 5)
